...
def test_joern_extract_issue():
    ...
    assert issues[0] == {
        ...
        "short_description": "Sensitive Data Leak: Security-sensitive data is leaked via `req` to log in `anonymous`",
...
def test_sample_convert(test_findings):
    data = convertLib.report(issues=test_findings)
